...
export default class Table extends Widget {
  constructor() {
    ...
    this.groupingStyle = Table.GroupingStyle.TITLE;
    this.header = null;
  ...
  static GroupingStyle = {
    /**
     * Aggregation rows in table are displayed on top of a row group as a title (aggregated column texts left like a header)
     */
    TITLE: 'title',
    /**
  ...
  _isGroupingOnTop() {
    return this.groupingStyle === Table.GroupingStyle.TOP || this.groupingStyle === Table.GroupingStyle.TITLE;
  }

  _group(animate) {
    let rows, nextRow, newGroup, firstRow, lastRow,
      groupColumns = this._groupedColumns(),
      onTop = this._isGroupingOnTop(),
      states = [];
  ...
  _renderAggregateRows(animate) {
    ...
    this._aggregateRows.forEach(function(aggregateRow, r) {
      ...
      $aggregateRow.toggleClass('table-aggregate-row-title', this.groupingStyle === Table.GroupingStyle.TITLE);
  ...
  _resizeAggregateCell($cell) {
    if (this.groupingStyle === Table.GroupingStyle.TITLE) {
      // in title mode a resize of the aggregate column might also be necessary if the column itself does not contain any content
      ...
      // closest to $cell.
      let $range = this._getTitleAggrCellRange($cell);
      this._updateTitleAggrCell($range[0]);
      this._updateTitleAggrCell($range[$range.length - 1]);
    } else {
      // hide icon if there is not enough space for aggregate text and icon
   ...
   */
  _updateTitleAggrCell($cell) {
    let $cellText = $cell.children('.text');
    if (!$cellText || !$cellText.length || $cell.hasClass('empty')) {
      return; // nothing to update (empty cell)
    }

    let iconWidth = 8 + 10; // TODO: ask cgu
    let $icon = $cell.children('.table-cell-icon').first();
    let cellWidth = this._getTitleAggrCellWidth($cell, $icon, iconWidth);
    cellWidth = this._updateTitleAggrIconVisibility($icon, $cellText, cellWidth, iconWidth);
   ...
   * @param {number} iconWidth Width of the cell icon including margins
  ...
  _getTitleAggrCellWidth($cell, $icon, iconWidth) {
    ...
    for (let i = 1; i < $range.length; i++) {
      ...
      let aggrCellWidth = $aggrCell.cssMaxWidth();
      if ($aggrCell.hasClass('empty')) {
        // cell is empty. its space may be used completely
        cellSpaceForText += aggrCellWidth;
      } else if (growsLeft ^ $aggrCell.hasClass('halign-right')) {
        // cells grow towards each other: use the free space of the cell
        let aggrCellConsumedSpace = $aggrCell.children().toArray()
          .map(item => $(item))
          .filter($item => $item.isVisible())
          .map($item => graphics.size($item, {includeMargin: true}).width)
          .reduce((a, b) => a + b, 0);
        let freeSpace = aggrCellWidth - aggrCellConsumedSpace - $cell.cssPaddingLeft();
        cellSpaceForText += freeSpace;
      }
    }
   ...
   */
  _updateTitleAggrIconVisibility($icon, $cellText, spaceAvailableForText, iconWidth) {
    if (!$icon || !$icon.length) {
      return spaceAvailableForText;
    }
    let isIconVisible = $icon.isVisible();
    let cellTextWidth = graphics.size($cellText, {includeMargin: true}).width;
